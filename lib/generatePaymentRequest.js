...
module.exports = function(millisatoshis, options) {
	...
	let { description, descriptionHash, paymentSecret } = options;
	...
	if (paymentSecret) {
		assert.ok(Buffer.isBuffer(paymentSecret), 'Invalid "paymentSecret": Buffer expected');
		assert.strictEqual(paymentSecret.length, 32, 'Invalid "paymentSecret": Must be 32 bytes');
		tags.push({
