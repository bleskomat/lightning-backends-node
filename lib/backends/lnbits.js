...
class Backend extends HttpLightningBackend {
	...
	addInvoice(amount, extra) {
		return this.request('post', '/api/v1/payments', {
			...
			unhashed_description: Buffer.from(extra.description, 'utf8').toString('hex'),
	...
	inputs: [
		{
			name: 'baseUrl',
			label: 'Base URL',
			help: 'The URL of the LNBits instance that you wish to connect. Please note that the legend.lnbits.com instance is unstable and should only be used for testing or a quick demonstration.',
			type: 'text',
		...
		},
		{
			name: 'adminKey',
			label: 'Admin Key',
			help: 'From your wallet page, open "API info" to view the "Admin key"',
			type: 'password',
			placeholder: 'xxx',
			default: '',
			required: true,
