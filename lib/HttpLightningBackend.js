...
class HttpLightningBackend extends LightningBackend {
	...
	request(method, uri, data, extraHeaders = {}) {
		return Promise.resolve().then(() => {
			...
			const finalHeaders = Object.assign(Object.assign({}, headers), extraHeaders);
			let options = {
			...
			if (data && (options.method === 'POST' || options.method === 'PUT' || options.method === 'PATCH')) {
			...
			return new Promise((resolve, reject) => {
				const done = (error, result) => {
					if (error) return reject(new Error(`HttpLightningBackend [${name}] Request Failed:\n${options.method} ${requestUrl} ${data}\n\n${error.message}`));
					resolve(result);
				};
				...
				const req = request(options, res => {
					...
					res.on('end', () => {
						...
						try { body = this.validateResponse(res, body); } catch (error) {
							return done(error);
						}
						done(null, body);
					});
	...
	validateResponse(res, body) {
		assert.ok(res.statusCode < 300, `Unexpected HTTP response status: ${res.statusCode}\n${body}`);
		if (this.options.responseType === 'json' || res.headers['content-type'].split(';')[0] === 'application/json') {
			try { body = body && JSON.parse(body) || null; } catch (error) {
				throw new Error('Failed to parse response body: JSON data expected');
			}
		}
...
HttpLightningBackend.prototype.checkMethodErrorMessages = {
	payInvoice: {
		ok: [],
		notOk: [
